...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#FFF; padding:20px;">
  <tr>
    ...
    $cat = $_POST['cat'];
	?>
    <div id="con50"><h2 class="banquet">Order List @ <?php echo $shopBranch[$branch_id];?></h2></div><div id="con50" align="right"></div>
    <br clear="all" />
...
?>
  <tr>
    <td align="center" class="tbDesc<?php if($data['ord_chk']!='yes'){echo "New";} ?>"><?php if($data['ord_chk']!='yes'){?> <object id="FlashID" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="50" height="20">
    ...
    <td align="center" class="tbDesc<?php if($data['ord_chk']!='yes'){echo "New";} ?>"><?php echo $titleOrderID.str_pad($data['id'],6,"0",STR_PAD_LEFT);?></td>
    <td align="center" class="tbDesc<?php if($data['ord_chk']!='yes'){echo "New";} ?>"><?php echo $data['ord_cname'];?></td>
    <td align="center" class="tbDesc<?php if($data['ord_chk']!='yes'){echo "New";} ?>"><?php echo $data['ord_date'];?></td>
    <td align="center" class="tbDesc<?php if($data['ord_chk']!='yes'){echo "New";} ?>"><a href="_orderDetail.php?id=<?php echo $data['id']; ?>" target="_blank" class="txtRed">Show Detail</a></td>
    <td align="center" class="tbDesc<?php if($data['ord_chk']!='yes'){echo "New";} ?>"><?php if($data['ord_choicedelivery']!=1) { echo $data['ord_addr']." (".$area.")";} else {echo $area;}?></td>
    <!--<td align="center" class="tbDesc<?php if($data['ord_chk']!='yes'){echo "New";} ?>"><?php if($data['ord_ordertime']==''){ echo "DINNER";} else { echo strtoupper($data['ord_ordertime']);} ?></td>-->
    <td align="center" class="tbDesc<?php if($data['ord_chk']!='yes'){echo "New";} ?>">
    
    <?php if($data['timepickup']==''){?>
    
    <form id="regis<?php echo $data['id']; ?>" name="regis<?php echo $data['id']; ?>" method="post" action="admin/sendmailtime-sys.php?id=<?php echo $data['id']; ?>&condition=<?php echo $condition;?>&branch_id=<?php echo $branch_id; ?>">
    
        <select name="time<?php echo $data['id']; ?>" id="time<?php echo $data['id']; ?>" style="width:80px;">
    ...
    <?php } else {  echo $data['timepickup']; }?>    
    
    </td>
    <td align="center" class="tbDesc<?php if($data['ord_chk']!='yes'){echo "New";} ?>"><a href="#" onClick="ConfirmChoice('<?php echo $data['id'];?>', '<?php echo $condition; ?>', '<?php echo $branch_id; ?>'); return false;"><img src="images/bt-del.png" border="0" width="40" align="absmiddle" /></a></td>
  </tr>
<?php } ?>
</table>
