...
<?php
	if(isset($_POST['boxUser']) && !empty($_POST['boxUser']) && (isset($_POST['boxPw']) && !empty($_POST['boxPw']))){
		...
		if($verified){
			...
			?>
			<SCRIPT LANGUAGE="JavaScript">window.location="inc/admin/pick_branch.php";</script>
		<?php } else { ?>
		...
		<SCRIPT LANGUAGE="JavaScript">window.location="login.php?user=<?php echo $user;?>&pw=<?php echo $pw;?>";</script>
		<?php
	}
