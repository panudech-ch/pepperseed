...
<script type="text/javascript">
function chkform(){
		<? if($_SESSION['area']!=1) {  // 1 = Pick Up at Shop ?>
...
<form id="regis" name="regis" method="post" action="menu_addOrderNo.php?branch_id=<? echo $branch_id; ?>" onsubmit="return chkform();">
<table width="100%" border="0" cellpadding="0" cellspacing="0">
<? if($_SESSION['area']!=1) {  // 1 = Pick Up at Shop ?>
  <tr>
    <td width="150" align="right" valign="top"><span class="txtCmmt">*</span> Address :</td>
    <td>
      <textarea name="address" id="address" rows="5" style="width:300px; margin-left:3px;"><? echo $_SESSION['address'];?></textarea>
      </td>
  </tr>
  <tr>
    <td align="right" valign="top"><span class="txtCmmt">*</span> Suburb :</td>
    <td><div>
<select name="area" id="area" onChange="javascript:DeliveryChoice(this.value, document.getElementById('address').value, document.getElementById('cname').value, document.getElementById('email').value, document.getElementById('phone').value, document.getElementById('comment').value, <? echo $branch_id; ?>);" style="width:300px; margin-left:3px; font-size:14px;">
		...
		?>        
        	<option value="<? echo $item['id'];?>" <? if($_SESSION['area']==$item['id']){echo "selected='selected'";}?>><? echo $item['area'];?> 
            <? if($item['charge_price']>0){ echo "($".number_format($item['charge_price'],2).")"; } ?>
            </option>
        <? } ?>
      </select>
</div></td>
  ...
  <tr>
    ...
    <td>
      <input type="text" name="cname" id="cname" class="txtBox" style="width:300px;" value="<? echo $_SESSION['cname'];?>"/>
    </td>
  </tr>
  <tr>
    <td align="right" valign="top"><span class="txtCmmt">*</span> Email :</td>
    <td>
      <input type="text" name="email" id="email"  class="txtBox" style="width:300px;" value="<? echo $_SESSION['email'];?>"/><br />
<i class="txtCmmt">*Confirmation to this email</i>
  ...
  <tr>
    ...
    <td>
      <input type="text" name="phone" id="phone"  class="txtBox" style="width:300px;" value="<? echo $_SESSION['phone'];?>"/>
    </td>
  </tr>
  <tr>
    <td align="right" valign="top"> Special Remark  :</td>
    <td>
      <textarea name="comment" id="comment" rows="5" style="width:300px; margin-left:3px;"><? echo $_SESSION['comment'];?></textarea>
    </td>
  </tr>
  <tr>
    <td align="right" valign="top">&nbsp;</td>
...
<? } else if ($_SESSION['ordertime']=='lunch'){ ?>    
...
<? } ?></td>
  ...
  <tr>
    ...
    <td>
		<label><input type="radio" name="pay" value="Cash" <? if(($_SESSION['payment']=="Cash") or ($_SESSION['payment']=='')){ echo "checked='checked'"; } ?>/>
		  <strong>Cash</strong></label>
		<br />
        
		<label><input type="radio" name="pay" value="Master Card" <? if($_SESSION['payment']=="Master Card"){ echo "checked='checked'"; } ?>/>
		  <strong>Master Card <? if($surchargePercent!=0){?>
          <? if($subTotal<=20){?>
          	<b class="txtCmmt">($<?  $surcharge = $subTotal*($surchargePercent/100);   $tmpShow=$subTotal+$surcharge; echo number_format($tmpShow,2);?>)</b>
            <i style='color:#e7585d; line-height:10px; font-size:16px;'><br />&nbsp;&nbsp;&nbsp; &nbsp; ( <strong><? echo $surchargePercent; ?>%</strong> apply for transaction less than <strong>$20</strong> )</i>	
			<? } ?></strong></label><!----></span>
          <? } ?>
        <? if($area!="Pick Up at Shop"){?>
        ...
        <? } else { echo " <i style='color:#e7585d; line-height:10px;''><br/>&nbsp;&nbsp;&nbsp; &nbsp; (Please present card at shop)<br /><br /></i>"; } ?>
            
		<label><input type="radio" name="pay" value="Visa" <? if($_SESSION['payment']=="Visa"){ echo "checked='checked'"; } ?>/>
		  <strong>Visa <? if($surchargePercent!=0){?>
          <? if($subTotal<=20){?>
          	<b class="txtCmmt">($<?  $surcharge = $subTotal*($surchargePercent/100);   $tmpShow=$subTotal+$surcharge; echo number_format($tmpShow,2);?>)</b>
            <i style='color:#e7585d; line-height:10px; font-size:16px;'><br />&nbsp;&nbsp;&nbsp; &nbsp; ( <strong><? echo $surchargePercent; ?>%</strong> apply for transaction less than <strong>$20</strong> )</i>			
			<? } ?></strong></label><!----></span>
		...
		<!--
        <label><input type="radio" name="pay" value="Eftpos" <? if($_SESSION['payment']=="Eftpos"){ echo "checked='checked'"; } ?>/>
		  <strong>Eftpos <? if($surchargePercent!=0){?>( Surcharge <? echo $surchargePercent; ?>% ) <b class="txtCmmt">($<?  $surcharge = $subTotal*($surchargePercent/100);   $tmpShow=$subTotal+$surcharge; echo number_format($tmpShow,2);?>)</b><? } ?></strong></label><br />
        ...
        <label><input type="radio" name="pay" value="AMEX" <? if($_SESSION['payment']=="AMEX"){ echo "checked='checked'"; } ?>/>
		  <strong>AMEX <? if($surchargePercent!=0){?>( Surcharge <? echo $surchargePercent; ?>% ) <b class="txtCmmt">($<?  $surcharge = $subTotal*($surchargePercent/100);   $tmpShow=$subTotal+$surcharge; echo number_format($tmpShow,2);?>)</b><? } ?></strong></label>
        <br /></span>
