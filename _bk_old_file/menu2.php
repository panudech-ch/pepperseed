...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <?php if($show!='yes'){?><div style="color:#000;"><strong>Pick Up <?php if($_SESSION['ordertime']=='lunch') { echo "Only";} else { echo "or Delivery"; }?></strong>  <br>
<select name="area" id="area" onChange="javascript:DeliveryChoice(this.value);" style="width:100%; font-size:14px;">
		...
		?>        
        	<?php if($_SESSION['ordertime']=='lunch') {?>
            	<?php if($item['area']!='Home Delivery'){?>
        		<option value="<?php echo $item['id'];?>" <?php if($_SESSION['area']==$item['id']){echo "selected='selected'";}?>><?php echo $item['area'];?></option>
                <?php } ?>
            <?php } else { ?>
        		<option value="<?php echo $item['id'];?>" <?php if($_SESSION['area']==$item['id']){echo "selected='selected'";}?>><?php echo $item['area'];?></option>
            <?php } ?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <td background="images/onlineTotalC.png" align="right" style="padding-right:15px;"><span class="txtYellow" style="font-size:20px;">Total $<?php echo number_format($subTotal,2);?></span></td>
    <td width="10"><img src="images/onlineTotalR.png" ></td>
  </tr>
</table>
<?php if(($subTotal > 0) && ($show!='yes')) { ?>
<div align="center" style="margin:10px 0;"><input name="btnCheckOut" type="image" src="images/onlineBTCheckout.png" id="btnCheckOut" value="Check Out" onClick="Javascript:chkform(document.getElementById('area').value, <?php echo $minimumPrice?>, <?php echo $subTotal; ?>, '<?php echo $canOrderUnderMinPrice; ?>' );"></div>
<?php } ?>
<?php if($show=='yes') { ?>
<div align="center" style="margin:10px 0;"><a href="menu.php?branch_id=<?php echo $branch_id;?>&ordertime=<?php echo $_SESSION['ordertime']; ?>"><img src="images/onlineBTBacktoCart.png" border="0" ></a></div>
<?php } ?>
...
?>
  <tr>
    <td valign="top" class="lineList" width="20" align="center" style="line-height:14px;"><b class="txtRed" title="Change your quantity"><span class="ShowInputbox<?php echo $intRows;?>" onClick="Javascript:fncShowQTY('<?php echo $intRows;?>','<?php echo $data["id"];?>','<?php echo $data["p_qty"];?>');") style="cursor: pointer;"><?php echo $data["p_qty"];?></span></b></td>
    <td valign="top" class="lineList" style="padding:5px 5px 0 5px;line-height:14px;" width="8">x</td>
    <td valign="top" class="lineList" style="line-height:14px;"><?php echo $dataPro["p_name"];?>
    <?php    
				$choicename ="";
				$choicename2="";
				$pp=$pp1=$pp2 =0;
    ...
    <td width="60px;" align="right" valign="top" class="lineList">$<?php echo number_format($pp,2);?></td>
    <?php if($show!='yes'){?>
    <td align="right" valign="top" class="lineList" style="width:25px;">
    <a href="javascript:deleteConfirm('DELETE','<?php echo $data["id"];?>','<?php echo $dataPro["p_name"];?> <?php if($choicename!=''){ echo "(".$choicename.")";}?> <?php if($choicename2!=''){ echo "(".$choicename2.")";}?>');"><img src="images/onlineBTDel.png" border="0"></a>
    </td>
	<?php }?>
  </tr>
<?php }  ?>
  ...
  <tr>
    ...
    <td align="right" style="padding:15px 0 0 0;"><b>$<?php echo number_format($subTotal,2);?></b></td>
    <td>&nbsp;</td>
  </tr>
  ...
  ?>
  <tr>
    <td colspan="3" align="right" style="padding:5px 0 0 0;">Discount <?php echo $discount; ?>% </td>
    <td align="right" style="padding:5px 0 0 0;">- $<?php echo number_format($sumdiscount,2);?></td>
    <td>&nbsp;</td>
  </tr>
  <?php } ?>
  ...
  <tr>
    ...
    <td align="right" style="padding:5px 0 0 0;"> $<?php echo number_format($OrderUnderMinPrice,2);?></td>
    <td>&nbsp;</td>
  </tr>
  <?php } ?>
  ...
  <?php if($area!=''){ ?>
  <tr>
    <td colspan="4" align="right" style="padding:5px 0 0 0;">Delivery Area ( <i class="txtRed"><?php echo $area; ?> <?php if($charge_price>0){ echo "($".number_format($charge_price,2).")";  $subTotal+=$charge_price;  } ?></i> ) </td>
