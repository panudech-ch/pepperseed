...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="../css/custom_print.css" media="print" />
...
<?php
$id = $_GET['id'];
$mode = isset($_GET['mode']) ? isset($_GET['mode']) : '';
    ...
    <?php for ($i = 1; $i <= 1; $i++) {
        ...
        <tr>
            ...
            <?php if (($ord_payment != "Cash") && ($ord_payment != "")) {
                ...
                if(!strpos($arrPay[1],'CVV')){
                    $exp = substr($arrPay[1], -7, 6);
                }else{
                    $exp = substr($arrPay[1], -16, 5);
                }
                if($mode == 'print'){
                    $cvv = 'XXX';
                }else{
                ...
                $payNo = array();
                array_push($payNo,substr(trim($arrNo[0]),0,4));
                array_push($payNo,substr(trim($arrNo[0]),5,4));
                array_push($payNo,substr(trim($arrNo[0]),10,4));
                array_push($payNo,substr(trim($arrNo[0]),15,4));
                $printNew = '<span class="text-font-bold">'.$pay .'</span> <br>No. <span class="text-font-bold" style="font-size:12px;">' . $payNo[0].' '.$payNo[1].' '.$payNo[2].' '.$payNo[3] . '</span><br>Expiry Date <span class="text-font-bold" style="font-size:9px;">'.$exp.'</span><br>CVV <span class="text-font-bold" style="font-size:9px;">'.$cvv.'</span>';
                ?>
...
//                                            $choiceprice += $item4['choice_price'];
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top: 20px;">
                    ...
                    if($total_charge > 0){
                        ...
                        <tr>
                            ...
                            <td align="right" style="padding:5px 0 0 0;" class="text-font">Charge order total < $35</td>
